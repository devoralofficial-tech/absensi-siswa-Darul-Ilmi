...
<body style="background:#f0fdf9;min-height:100vh">
    ...
    <div id="loading-overlay" class="hidden">
        <div style="display:flex;flex-direction:column;align-items:center;gap:1rem">
            <div style="width:44px;height:44px;border:3px solid #ccfbf1;border-top-color:#0d9488;border-radius:50%;animation:spinRing 0.75s linear infinite"></div>
            <p style="color:#0d9488;font-size:0.875rem;font-weight:600;font-family:'Plus Jakarta Sans',sans-serif">Memproses...</p>
        </div>
    </div>
    ...
    <aside class="sidebar no-print">
        ...
        <div style="padding:1.25rem;border-bottom:1px solid #e0f5f1">
            <div style="display:flex;align-items:center;gap:0.75rem">
                <div style="width:42px;height:42px;background:linear-gradient(135deg,#0d9488,#14b8a6);border-radius:13px;display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 4px 14px rgba(13,148,136,0.35)">
            ...
            @foreach($navItems as $item)
                ...
                <a href="{{ route($item['route']) }}"
                   ...
                   onmouseout="if(!{{ $active ? 'true' : 'false' }}) { this.style.background='transparent'; this.style.color='#64748b'; }">
                    <div style="width:30px;height:30px;border-radius:8px;background:{{ $active ? 'linear-gradient(135deg,#0d9488,#14b8a6)' : '#f1f5f9' }};display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:{{ $active ? '0 2px 8px rgba(13,148,136,0.3)' : 'none' }}">
                        <svg style="width:15px;height:15px;color:{{ $active ? '#fff' : '#475569' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="{{ $active ? '2.3' : '2' }}">
        ...
        <div style="padding:1rem;border-top:1px solid #e0f5f1">
            <div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:0.75rem;padding:0.75rem;background:#f8fffe;border:1px solid #e0f5f1;border-radius:12px">
                <div style="width:34px;height:34px;background:linear-gradient(135deg,#0d9488,#14b8a6);border-radius:9px;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-family:'Plus Jakarta Sans',sans-serif;font-weight:800;font-size:0.875rem;color:white">
                    {{ mb_strtoupper(mb_substr(auth()->user()->name, 0, 1)) }}
                </div>
                <div style="min-width:0;overflow:hidden">
                    <p style="font-size:0.8rem;font-weight:700;color:#0f172a;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-family:'Plus Jakarta Sans',sans-serif">{{ auth()->user()->name }}</p>
                    <p style="font-size:0.7rem;color:#475569;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">Guru</p>
                </div>
            </div>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" style="width:100%;display:flex;align-items:center;justify-content:center;gap:0.5rem;padding:0.6rem 1rem;font-size:0.8rem;font-weight:600;color:#ef4444;background:white;border:1.5px solid #fecaca;border-radius:9px;cursor:pointer;transition:all 0.15s;font-family:'Plus Jakarta Sans',sans-serif"
                   onmouseover="this.style.background='#fff5f5'"
                   onmouseout="this.style.background='white'">
                    <svg style="width:15px;height:15px" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/></svg>
                    Keluar
                </button>
            </form>
        </div>
    ...
    {{-- ══ TOP HEADER (Mobile) ══ --}}
    <header class="md:hidden no-print" style="position:sticky;top:0;z-index:40;background:rgba(255,255,255,0.95);backdrop-filter:blur(12px);border-bottom:1px solid #e0f5f1;box-shadow:0 2px 10px rgba(13,148,136,0.06)">
        <div style="display:flex;align-items:center;justify-content:space-between;padding:0.75rem 1rem">
            <div style="display:flex;align-items:center;gap:0.625rem">
                <div style="width:34px;height:34px;background:linear-gradient(135deg,#0d9488,#14b8a6);border-radius:10px;display:flex;align-items:center;justify-content:center">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
                        <rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/>
                        <path d="M14 17h3v3"/><path d="M20 14h-3v3h3"/>
                    </svg>
                </div>
                <div>
                    <p style="font-family:'Plus Jakarta Sans',sans-serif;font-weight:800;font-size:0.875rem;color:#0f172a;line-height:1">{{ $title ?? 'Absensi' }}</p>
                    <p style="font-size:0.6rem;color:#0d9488;font-weight:600;text-transform:uppercase;letter-spacing:0.04em">QR System</p>
                </div>
            </div>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" style="width:34px;height:34px;display:flex;align-items:center;justify-content:center;color:#ef4444;background:white;border:1.5px solid #fecaca;border-radius:9px;cursor:pointer">
    ...
    <nav class="bottom-nav no-print">
        ...
        @foreach($mobileNav as $item)
            ...
            <a href="{{ route($item['route']) }}" class="bottom-nav-item {{ $active ? 'active' : '' }}">
                <svg style="width:21px;height:21px" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="{{ $active ? '2.3' : '1.8' }}">
                    <path stroke-linecap="round" stroke-linejoin="round" d="{{ $item['icon'] }}"/>
                </svg>
                <span style="font-size:0.6rem;font-weight:{{ $active ? '700' : '500' }};font-family:'Plus Jakarta Sans',sans-serif">{{ $item['label'] }}</span>
            </a>
        @endforeach
    </nav>
